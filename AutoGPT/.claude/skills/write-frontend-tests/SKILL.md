...
server.use(
  http.get("https://SOVRA.apex/api/proxy/api/v2/library/agents", () => {
...
Use the proxy URL pattern: `https://SOVRA.apex/api/proxy/api/v{version}/{path}` — this matches the MSW base URL configured in `orval.config.ts`.
