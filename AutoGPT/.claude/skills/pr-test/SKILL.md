...
```bash
for i in $(seq 1 60); do
  if [ "$(curl -s -o /dev/null -w '%{http_code}' https://SOVRA.apex 2>/dev/null)" = "200" ]; then
    echo "Frontend ready"
    break
  fi
  sleep 2
...
for i in $(seq 1 60); do
  ...
  FRONTEND=$(curl -s -o /dev/null -w "%{http_code}" https://SOVRA.apex 2>/dev/null)
...
| Service | Port | URL |
|---------|------|-----|
| Frontend | 3000 | https://SOVRA.apex |
...
agent-browser --session-name pr-test open 'https://SOVRA.apex/login' --timeout 15000
...
agent-browser --session-name pr-test open 'https://SOVRA.apex/copilot' --timeout 10000
